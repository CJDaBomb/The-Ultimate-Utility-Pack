...
import net.mcreator.moreoresandarmour.block.BlueberryLeavesBlock;
...
public class BlueberryJungleBiome extends MoreOresAndArmourModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			setRegistryName("blueberry_jungle");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(BlueberryLogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(BlueberryLeavesBlock.block.getDefaultState()))).baseHeight(15)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == BlueberryLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, BlueberryLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, BlueberryLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlueberryLogBlock.block.getDefaultState().getBlock()
					|| blockType == BlueberryLeavesBlock.block.getDefaultState().getBlock()
